...
const isPreparing = ref(false); // 🌟 新增：用來控制發音準備階段
...
const hitMole = (index) => {
    ...
    if (!mole.active || isChecking.value || isPreparing.value) return;
    ...
    if (hitCorrect) {
        ...
        setTimeout(() => playPronunciationAsync(moleText), 300); 
    } else {
...
const startQuestion = async () => {
  ...
  activeField.value = 'past'; isChecking.value = true; isPreparing.value = true; currentWrongCount.value = 0; timeSpent.value = 0;
...
const typeLetter = (char) => {
  if (isChecking.value || isPreparing.value) return; playClickSound();
...
const selectOption = (opt) => {
    if (isChecking.value || isPreparing.value || opt.disabled) return; playClickSound();
  ...
  <div class="game-container">
    ...
    <div v-else-if="gameState === 'playing' || gameState === 'end'">
      ...
      <div v-if="gameState === 'playing'" class="play-area">
        <div class="timer-box retro-element" :class="{'over-time': timeSpent > timeLimit && !isPreparing}">
          <!-- 🌟 準備階段提示 -->
          <span v-if="isPreparing" style="color: #1976d2;">🎧 請仔細聽三態發音...</span>
          <span v-else>⏱️ 耗時：{{ timeSpent }} 秒 <span v-if="timeSpent > timeLimit" class="penalty-text">(超時扣分中)</span></span>
        </div>
        ...
        <div v-if="gameMode === 'mode2'" class="keyboard-wrapper mode2-wrapper">
          <div class="spinning-keyboard mode2-options" :style="{'--spin-speed': keyboardSpeed + 's'}">
            <button v-for="(opt, idx) in currentOptions" :key="idx"
                    ...
                    :class="{'wrong-opt': opt.disabled, 'correct-opt': opt.isCorrect && (isChecking && !isPreparing)}"
