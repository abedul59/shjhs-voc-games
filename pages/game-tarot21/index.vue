...
const config = ref({ card_set: '1', hp: 30, time_limit: 15, penalty: 5, win_damage: 10, blankCount: 3 });

const myPlayerRole = ref(null); 
const currentRoomId = ref(null);

const players = ref({
    p1: { id: '', name: '', hp: 30, maxHp: 30, cards: [], score: 0, status: 'playing' }, 
    p2: { id: '', name: '', hp: 30, maxHp: 30, cards: [], score: 0, status: 'playing' } 
});
...
onMounted(async () => {
  try {
    ...
    if (settings) {
        if (settings.tarot21_card_set) config.value.card_set = settings.tarot21_card_set;
        if (settings.tarot21_hp) config.value.hp = settings.tarot21_hp;
        if (settings.tarot21_time_limit) config.value.time_limit = settings.tarot21_time_limit;
        if (settings.tarot21_penalty) config.value.penalty = settings.tarot21_penalty;
        if (settings.tarot21_win_damage) config.value.win_damage = settings.tarot21_win_damage;
        if (settings.tarot21_blank_count !== undefined) config.value.blankCount = settings.tarot21_blank_count;
        // 🌟 讀取禁賽門檻
        if (settings.tarot21_max_escapes !== undefined && settings.tarot21_max_escapes !== null) maxEscapes.value = Number(settings.tarot21_max_escapes);
    }
...
const syncMyStateToDB = async () => {
    ...
    await supabase.from('tarot21_rooms').update(updateField).eq('id', currentRoomId.value);
};

const pollGameState = async () => {
    if (matchStatus.value !== 'playing' || isResolving.value) return;
    const { data } = await supabase.from('tarot21_rooms').select('*').eq('id', currentRoomId.value).single();
    if (data) {
...
const findMatch = async () => {
    ...
    const { data: rooms } = await supabase.from('tarot21_rooms').select('*').eq('status', 'waiting').eq('unit_info', unitKey).neq('p1_id', myId).order('id', { ascending: false }).limit(1);
    ...
    if (rooms && rooms.length > 0) {
        ...
        await supabase.from('tarot21_rooms').update({ p2_id: myId, p2_name: myName, status: 'playing', p2_state: p2InitialState }).eq('id', room.id);
        ...
        const { data: newRoom } = await supabase.from('tarot21_rooms').insert([{ 
        ...
        if (newRoom) {
            ...
            pollTimer = setInterval(async () => {
                ...
                const { data: checkRoom } = await supabase.from('tarot21_rooms').select('*').eq('id', currentRoomId.value).single();
...
const drawCard = async () => {
    ...
    players.value[myPlayerRole.value].cards.push({ id: cardId, state: 'typing', word: pureWord, points: pureWord.length });

    myTarget.value = {
        word: pureWord, zh: newCardData.zh, typedCount: 0,
        timeRemaining: config.value.time_limit, cardId: cardId,
        points: pureWord.length,
...
const subscribeToRoom = (roomId) => {
    roomSubscription = supabase.channel(`db_room_${roomId}`).on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'tarot21_rooms', filter: `id=eq.${roomId}` }, (payload) => {
        const d = payload.new;
        if (d.status === 'playing') {
    ...
    gameChannel = supabase.channel(`tarot21_cast_${roomId}`);
  ...
  <div class="tarot-root">
    ...
    <div v-else-if="matchStatus === 'setup' || matchStatus === 'waiting'" class="setup-overlay">
      <div class="rpg-dialog retro-element" style="max-width: 500px;">
        ...
        <h2 style="color:#ffeb3b;">{{ matchStatus === 'setup' ? '命運交鋒' : '等待對手入座...' }}</h2>
        
        <template v-if="matchStatus === 'setup'">
            <div class="rules-box">
                <p>1️⃣ <b>瘋狂狂飆制</b>：雙方無須輪流，隨時可以瘋狂抽牌累積點數！</p>
                <p>2️⃣ 目標是讓點數加總<b>最接近但不超過 21 點</b>。</p>
                <p>3️⃣ 當您覺得點數夠了，可按「停牌」，系統會等待對手也停牌或爆牌後自動開獎。</p>
                <p>4️⃣ <b>底牌隱密</b>：對方抽的牌皆為覆蓋狀態，雙方皆停牌結算時才會全部亮出！</p>
                <p style="color:#f44336;">⚠️ 拼錯單字會導致卡牌反噬自燃，並扣除自身血量！</p>
            </div>
          ...
          <div class="player-info retro-element my-info" style="margin-bottom: 10px;">
              <div style="display:flex; justify-content:space-between; align-items:center;">
                  ...
                  <div class="p-score" :class="{'busted-text': players[myPlayerRole === 'p1' ? 'p2' : 'p1'].status === 'busted'}">
                      點數: <span style="color:#ffeb3b; font-size:1.4rem;">{{ isRevealedAll ? players[myPlayerRole === 'p1' ? 'p2' : 'p1'].score : '??' }}</span>
