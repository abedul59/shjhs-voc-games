      ...
      <div class="content-layout">
        <main class="text-area">
          ...
          <div class="reading-window">
            ...
            <div v-show="activeTab === 'personal'" class="formatted-rich-text personal-font"><div v-html="noteData.personal_notes || '<p class=empty-hint>尚未新增筆記</p>'"></div></div>
          ...
          <div class="portal-group">
            ...
            <div class="portal-list">
              <button class="portal-btn" :disabled="!noteData.class_video_link" @click="openPortal(noteData.class_video_link)">
                <span class="icon">▶️</span><div class="label"><strong>影片摘要</strong></div>
              </button>
              <button class="portal-btn" :disabled="!noteData.notebook_audio_link" @click="openPortal(noteData.notebook_audio_link)">
                <span class="icon">🎧</span><div class="label"><strong>聲音摘要</strong></div>
              </button>
              <button class="portal-btn" :disabled="!noteData.document_link" @click="openPortal(noteData.document_link)">
                <span class="icon">📄</span><div class="label"><strong>NotebookLM簡報</strong></div>
              </button>
              <button class="portal-btn" :disabled="!noteData.notebook_infographic_link" @click="openPortal(noteData.notebook_infographic_link)">
                <span class="icon">📊</span><div class="label"><strong>資訊圖表</strong></div>
              </button>
              <button class="portal-btn" :disabled="!noteData.notebook_quiz_link" @click="openPortal(noteData.notebook_quiz_link)">
                <span class="icon">📝</span><div class="label"><strong>測驗 Quiz</strong></div>
              </button>
              <button class="portal-btn" :disabled="!noteData.notebook_flashcard_link" @click="openPortal(noteData.notebook_flashcard_link)">
                <span class="icon">📇</span><div class="label"><strong>學習卡</strong></div>
              </button>
              <button class="portal-btn" :disabled="!noteData.notebook_overall_link" @click="openPortal(noteData.notebook_overall_link)">
                <span class="icon">📁</span><div class="label"><strong>整體資源</strong></div>
              </button>
            </div>
          </div>
          <div class="sidebar-footer">更新於：{{ new Date(noteData.updated_at || noteData.created_at).toLocaleDateString() }}</div>
        </aside>
      </div>
    </template>
  </div>
...
.portal-group h3 { margin: 0 0 15px 0; font-size: 18px; color: #1e293b; border-bottom: 2px solid #3b82f6; padding-bottom: 8px;}
...
.portal-btn .label strong { font-size: 14px; color: #1e293b; }
.portal-btn:hover:not(:disabled) { transform: translateY(-2px); border-color: #1e3a8a; box-shadow: 0 4px 6px rgba(0,0,0,0.05); }
.portal-btn:disabled { opacity: 0.4; cursor: not-allowed; }
.sidebar-footer { margin-top: 20px; text-align: center; font-size: 11px; color: #94a3b8; }
.loading-overlay { text-align: center; padding: 100px; }
