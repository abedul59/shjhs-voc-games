...
const fetchNotes = async () => {
  ...
  // 🌟 只抓取「土地稅法」的筆記
  const { data, error } = await supabase.from('course_notes').select('id, topic, introduction, created_at').eq('subject', '土地稅法').order('created_at', { ascending: true });
...
<template>
  <div class="list-container">
    ...
    <div class="actions-bar">
      <NuxtLink to="/admin/law-exam/land-tax-law-course/edit" class="btn-add-new">
        ➕ 新增講次 / 筆記
      </NuxtLink>
    </div>
    ...
    <div v-else class="notes-grid">
      <div v-for="(note, index) in notes" :key="note.id" class="note-card">
        ...
        <div class="card-actions">
          <NuxtLink :to="`/admin/law-exam/land-tax-law-course/${note.id}`" class="btn-read">📖 閱讀</NuxtLink>
          <NuxtLink :to="`/admin/law-exam/land-tax-law-course/edit?id=${note.id}`" class="btn-edit">⚙️ 編輯</NuxtLink>
        </div>
      </div>
    </div>
  </div>
...
.header h1 { font-size: 32px; margin: 0 0 10px 0; color: #e11d48; }
.header p { color: #64748b; }

.actions-bar { display: flex; justify-content: center; margin-bottom: 40px; }
.btn-add-new { background: #e11d48; color: white; padding: 14px 28px; border-radius: 12px; font-size: 16px; font-weight: bold; text-decoration: none; box-shadow: 0 4px 10px rgba(225, 29, 72, 0.3); transition: transform 0.2s; }
...
.note-card:hover { border-color: #e11d48; box-shadow: 0 10px 15px -3px rgba(225, 29, 72, 0.1); }
.card-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px; }
.lecture-badge { background: #fff1f2; color: #be123c; padding: 4px 10px; border-radius: 6px; font-size: 12px; font-weight: bold; }
...
.btn-read { background: #e11d48; color: white; }
