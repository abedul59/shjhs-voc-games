...
const CONFIG = {
  tableName: 'teachers_leave_regulations_clauses',
  pageTitle: '教師請假規則',
  parentLawName: '教師法', // 🌟 子法設定：母法為教師法
  primaryColor: '#ec4899', hoverColor: '#db2777', lightBg: '#fdf2f8', activeBg: '#fbcfe8'
};

const supabase = useSupabaseClient();
const clauses = ref([]); const parentClausesMap = ref({}); const customLawMap = ref({}); 
const isLoading = ref(true); const viewMode = ref('single'); // 無章節
...
const handleContentClick = (e) => { 
  if (e.target.classList.contains('ref-btn')) { 
    ...
    // 🌟 在規則中，本規則指向自己，本法指向母法(教師法)！
    if (['self', '本規則', '本辦法'].includes(inheritedContext)) finalTargetLaw = CONFIG.pageTitle;
    ...
    if (target) floatingReference.value = { ...target, displayTitle, canJump: (finalTargetLaw === CONFIG.pageTitle) }; else alert(`無法定位：${finalTargetLaw} 第 ${convertedNum} 條`);
  } 
};
...
<style scoped>
/* 與 [施行細則] 完全相同的 CSS (略以節省空間，請直接複製第二點的 <style scoped>) */
...
.btn-tool.primary { background: var(--primary); color: white; border: none; border-radius:8px; cursor: pointer; width:100%; box-sizing:border-box;}
