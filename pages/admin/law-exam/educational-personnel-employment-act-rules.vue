...
const CONFIG = {
  tableName: 'educational_personnel_employment_act_rules_clauses', // 記得在 Supabase 建立此表
  pageTitle: '教育人員任用條例施行細則',
  parentLawName: '教育人員任用條例', // 🌟 重點：指向母法！
  primaryColor: '#0891b2',
  hoverColor: '#0e7490',
  lightBg: '#cffafe',
  activeBg: '#a5f3fc'
...
const fetchData = async () => {
  ...
  if (data) {
    clauses.value = data.map(c => ({ ...c, urls: Array.isArray(c.urls) ? c.urls : (typeof c.urls === 'string' ? JSON.parse(c.urls || '[]') : []) })).sort((a, b) => parseFloat(String(a.article_num || '0').replace('-', '.')) - parseFloat(String(b.article_num || '0').replace('-', '.')));
    ...
    Object.keys(LAW_TABLE_MAP).forEach(lawName => { const tName = LAW_TABLE_MAP[lawName]; if (tableDataMap[tName]) parentClausesMap.value[lawName] = tableDataMap[tName]; });
  }
  isLoading.value = false;
};
onMounted(fetchData);

const addCustomMap = async () => { if (!newMapWrong.value || !newMapCorrect.value) return; const wrong = newMapWrong.value.trim().replace(/\s/g, ''); const correct = newMapCorrect.value; customLawMap.value[wrong] = correct; await supabase.from('law_local_alias_mapping').upsert({ context_law: CONFIG.tableName, wrong_name: wrong, correct_name: correct }, { onConflict: 'context_law, wrong_name' }); newMapWrong.value = ''; };
...
const filteredFlatClauses = computed(() => { if (!searchQuery.value) return clauses.value; return clauses.value.filter(c => (c.title || '').includes(searchQuery.value) || (c.content || '').includes(searchQuery.value)); });
const chapterClauses = computed(() => { return filteredFlatClauses.value.filter(c => { const matchChap = (c.chapter_name || '未分類編') === selectedChapter.value; if (selectedSection.value) return matchChap && (c.section_name || '未分類章') === selectedSection.value; return matchChap; }); });
...
const parseNum = (str) => { 
  ...
  for (let i = 0; i < str.length; i++) { let val = dict[str[i]]; if (val === undefined) continue; if (val >= 10) { if (current === 0) current = 1; total += current * val; current = 0; } else current = val; } 
  return (total + current).toString(); 
};
const getNormalizedArticleNum = (rawText) => { let text = rawText.replace(/\s/g, ''); const match = text.match(/第(.+)條(?:之(.+))?/); if (!match) return null; return parseNum(match[1]) + (match[2] ? '-' + parseNum(match[2]) : ''); };

// 🌟 子母法連動升級：加入規則、要點等正則表達式
...
const handleContentClick = (e) => { 
  if (e.target.classList.contains('ref-btn')) { 
    ...
    // 判斷是指向自身，還是指向母法
    const selfKeywords = ['self', '本法', '本規則', '本細則', '本準則', '本要點', '本辦法'];
    // 🌟 在細則中遇到「本條例」，直接轉向母法！
    const parentKeywords = ['本條例', '該法', '同法', '前法']; 
    ...
    if (finalTargetLaw === CONFIG.pageTitle) {
      ...
      } else { alert(`📚 找不到「${finalTargetLaw}」的資料。請在左側校正字典對應！`); return; }
    }
