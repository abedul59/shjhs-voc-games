...
// 🌟 配置：民法與信託法
const form = ref({
  id: null, subject: '民法與信託法', topic: '', introduction: '', class_video_link: '', notebook_audio_link: '', document_link: '', notebook_infographic_link: '', notebook_quiz_link: '', notebook_flashcard_link: '', notebook_overall_link: '', summary: '', transcript: '', personal_notes: '', associated_urls: ''
});
...
<template>
  <div class="edit-container">
    <div class="header">
      ...
      <div class="title-wrap"><h1>筆記編輯器</h1><span class="subject-tag">{{ form.subject }}</span></div>
    </div>
    <div class="tabs-nav">
    ...
    <div v-else class="form-body">
      <section v-show="activeTab === 'basic'" class="tab-content">
        <div class="form-section">
          <h3>講次基本資訊</h3>
          <div class="form-group"><label>主題名稱 (必填)</label><input v-model="form.topic" type="text" class="input-field" placeholder="例如：信託行為之要件" /></div>
      ...
      <section v-show="activeTab === 'study'" class="tab-content">
        <div class="form-section">
          ...
          <div v-for="(note, index) in personalNotesList" :key="index" class="personal-note-wrapper">
            <div class="note-header"><span class="note-badge">區塊 {{ index + 1 }}</span><button @click.prevent="removePersonalNote(index)" class="btn-remove-note">🗑️</button></div>
...
.subject-tag { background: #6366f1; color: white; padding: 4px 10px; border-radius: 6px; font-size: 13px; font-weight: bold; }
...
.tabs-nav button.active { background: #6366f1; color: white; }
...
.form-section h3 { margin: 0 0 20px 0; font-size: 18px; color: #1e293b; border-left: 5px solid #6366f1; padding-left: 12px; }
...
.personal-bg { background: #e0e7ff; border-color: #c7d2fe; min-height: 200px;}
.btn-add-note { display: block; width: 100%; padding: 12px; background: #e0e7ff; color: #4f46e5; border: 2px dashed #c7d2fe; border-radius: 10px; font-weight: bold; font-size: 15px; cursor: pointer; transition: 0.2s;}
.btn-add-note:hover { background: #c7d2fe; }
...
.save-btn { background: #6366f1; color: white; border: none; padding: 15px 40px; border-radius: 12px; font-size: 16px; font-weight: bold; cursor: pointer; box-shadow: 0 4px 10px rgba(99, 102, 241, 0.3); }
