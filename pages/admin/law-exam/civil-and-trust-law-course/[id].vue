  ...
  <div class="reader-container">
    ...
    <template v-else-if="noteData">
      <div class="reader-header">
        <div class="nav-bar">
          <NuxtLink to="/admin/law-exam/civil-and-trust-law-course" class="btn-back">← 回列表</NuxtLink>
          <NuxtLink :to="`/admin/law-exam/civil-and-trust-law-course/edit?id=${noteData.id}`" class="btn-edit">⚙️ 編輯</NuxtLink>
        </div>
        <div class="title-section"><span class="subject-label">{{ noteData.subject }}</span><h1>{{ noteData.topic }}</h1></div>
      </div>
...
.btn-edit { background: #e0e7ff; color: #4f46e5; border: 1px solid #c7d2fe; }
.subject-label { background: #6366f1; color: white; padding: 4px 12px; border-radius: 6px; font-size: 14px; font-weight: 900; }
...
.intro-card { background: #e0e7ff; border-left: 6px solid #4f46e5; padding: 20px; border-radius: 12px; margin-bottom: 25px; }
...
.tabs-nav button.active { background: white; color: #6366f1; border-top: 3px solid #6366f1; }
...
:deep(.urls-font a:hover) { background: #e0e7ff; border-color: #c7d2fe; }

.portal-sidebar { background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 20px; padding: 20px; position: sticky; top: 20px; }
.portal-group h3 { margin: 0 0 15px 0; font-size: 18px; border-bottom: 2px solid #6366f1; padding-bottom: 8px;}
...
.portal-btn:hover:not(:disabled) { border-color: #6366f1; box-shadow: 0 4px 6px rgba(0,0,0,0.05); }
