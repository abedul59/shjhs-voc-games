...
onMounted(async () => {
  ...
  if (noteId) {
    ...
    if (error) {
      ...
      router.push('/admin/law-exam/administrative-course');
      return; // 提早結束
    }
  } else if (lessonNum) {
    // 新增模式：自動帶入堂數
...
const saveNote = async () => {
  ...
  if (isEditing.value && form.value.id) {
    ...
    if (error) {
      ...
      router.push(`/admin/law-exam/administrative-course/${form.value.id}`);
    }
  } else {
    // 插入新資料
    const { data, error } = await supabase
    ...
    if (error) {
      ...
      router.push(`/admin/law-exam/administrative-course/${data.id}`);
    }
  }
...
.subject-tag { background: #374151; color: white; padding: 4px 10px; border-radius: 6px; font-size: 13px; font-weight: bold; }
