...
const currentSubject = '行政法'; // 🌟 已更改為行政法
  ...
  <div class="course-grid-container">
    <div class="header">
      ...
      <div class="title-area">
        <span class="subject-tag admin-tag">{{ currentSubject }}</span>
    ...
    <div v-else class="grid-layout">
      <div v-for="item in gridItems" :key="item.lessonNumber" class="grid-card" :class="{ 'has-data': item.hasData }">
        ...
        <div class="card-action">
          <NuxtLink v-if="item.hasData" :to="`/admin/law-exam/administrative-course/${item.id}`" class="action-btn view-btn">📖 閱讀</NuxtLink>
          <NuxtLink v-else :to="`/admin/law-exam/administrative-course/edit?lesson=${item.lessonNumber}`" class="action-btn add-btn">➕ 新增</NuxtLink>
        </div>
      </div>
    </div>
  </div>
...
.subject-tag.admin-tag { background: #475569; color: white; padding: 6px 14px; border-radius: 8px; font-weight: 900; }
...
.grid-card.has-data { border-color: #cbd5e1; background: #f8fafc; }
.grid-card.has-data:hover { border-color: #475569; transform: translateY(-5px); }
...
.view-btn { background: #475569; color: white; }
.add-btn { background: white; color: #475569; border: 1px solid #475569; }
</style>
