...
const fetchData = async () => {
  ...
  const [childRes, parentRes] = await Promise.all([
    ...
    supabase.from('teachers_act_clauses').select('*')
  ]);

  if (childRes.error) alert('無法連線資料庫：' + childRes.error.message);
  ...
  if (parentRes.data) {
    parentClauses.value = parentRes.data; // 儲存母法資料供跳轉使用
  }
...
const handleContentClick = (e) => { 
  if (e.target.classList.contains('ref-btn')) { 
    ...
    if (target) {
      ...
      alert(`無法定位：${rawText} (條號 ${convertedNum})。請確認教師法資料已匯入。`);
    }
  } 
};
