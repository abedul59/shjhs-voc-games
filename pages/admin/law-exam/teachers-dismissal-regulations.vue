  ...
  return parts.map(part => {
    if (/^第\s*[0-9一二三四五六七八九十百千-]+\s*條/.test(part)) {
      ...
      if (lawMatches.length > 0) {
        ...
        if (!['方法', '無法', '依法', '合法', '修法', '用法'].includes(lastLaw)) currentContext = lastLaw;
      }
      return part; 
    }
  }).join(''); 
...
const handleContentClick = (e) => { 
  if (e.target.classList.contains('ref-btn')) { 
    const rawText = e.target.getAttribute('data-raw'); // e.g. 第十六條
    const inheritedContext = e.target.getAttribute('data-law'); // e.g. 本法
    let finalTargetLaw = '';

    // 🌟 1. 優先攔截：檢查「顯示文字」有沒有專屬設定
    if (customLawMap.value[rawText]) {
      finalTargetLaw = customLawMap.value[rawText];
    } 
    // 🌟 2. 次級攔截：檢查「代稱」有沒有設定
    else if (customLawMap.value[inheritedContext]) {
      finalTargetLaw = customLawMap.value[inheritedContext];
    }
    // 🌟 3. 預設邏輯
    else {
      if (['本法', '該法', '同法', '前法'].includes(inheritedContext)) finalTargetLaw = CONFIG.parentLawName;
      else if (['本辦法', '本細則', '本準則'].includes(inheritedContext)) finalTargetLaw = CONFIG.pageTitle;
    ...
    if (finalTargetLaw === CONFIG.pageTitle) {
      ...
      if (db) {
        ...
        alert(`📚 找不到「${finalTargetLaw}」庫。請在校正字典將「${rawText}」或「${inheritedContext}」對應到正確法律！`);
        return;
      }
    }
...
<template>
  <div class="law-layout" :style="cssVars" :class="{ 'mode-all': viewMode === 'all' }">
    <div class="mobile-nav">
      ...
      <div class="mode-toggle"><button @click="viewMode = 'single'" :class="{ active: viewMode === 'single' }">逐條</button><button @click="viewMode = 'all'" :class="{ active: viewMode === 'all' }">全覽</button></div>
    </div>
    <div class="sidebar" :class="{ 'mobile-open': showSidebar }">
      <div class="sidebar-header">
        ...
        <details class="admin-tools"><summary>⚙️ 管理與校正字典</summary>
          <div class="tools-panel">
            ...
            <div class="mapping-tool">
              <h4>🔗 條文指涉校正</h4>
              <p class="mapping-desc">若點擊錯誤，可將「顯示文字(如:第十六條)」或「代稱(如:本法)」對應至正確法律。</p>
              <div class="map-list" v-if="Object.keys(customLawMap).length > 0">
                <div v-for="(correct, wrong) in customLawMap" :key="wrong" class="map-item"><span>{{ wrong }} ➔ {{ correct }}</span><button @click="removeCustomMap(wrong)">✕</button></div>
              </div>
              <div class="map-add-form"><input v-model="newMapWrong" placeholder="錯誤字" /><select v-model="newMapCorrect"><option :value="CONFIG.pageTitle">【自身】</option><option v-for="law in uniqueLawNames" :key="law" :value="law">{{ law }}</option></select><button @click="addCustomMap" class="btn-add-map">新增</button></div>
            </div>
      ...
      <div class="tree-list">
        ...
        <div v-else v-for="(sections, chapter) in groupedClauses" :key="chapter" class="chapter-group">
          <details open><summary class="chapter-title">{{ chapter }}</summary><div v-for="(items, section) in sections" :key="section" class="section-group"><details open><summary class="section-title">{{ section }}</summary><div v-for="c in items" :key="c.id" class="clause-item" :class="{active: selectedClause?.id === c.id}"><input type="checkbox" v-model="selectedIds" :value="c.id" class="q-checkbox" /><span @click="selectClause(c)" class="clause-label">{{ c.title }}</span></div></details></div></details>
        </div>
      </div>
    </div>
    <div class="main-content">
      <div v-if="viewMode === 'single'">
        <div v-if="!selectedClause" class="empty-state"><div class="empty-box"><h2>{{ CONFIG.pageTitle }}</h2><p>請從目錄選擇法條</p><button class="btn-open-menu-large" @click="showSidebar = true">開啟目錄</button></div></div>
        <div v-else class="clause-detail">
          <div class="clause-header"><h1>{{ selectedClause.title }}</h1></div>
      ...
      <div v-else class="full-text-view"><div class="clauses-container"><div v-for="c in filteredFlatClauses" :key="c.id" class="full-clause-card"><div class="card-side"><span class="card-num">{{ c.title }}</span><button @click="selectClause(c)">📝 編輯</button></div><div class="card-main"><div class="card-content" v-html="parseContentWithLinks(c.content)" @click="handleContentClick"></div><div v-if="c.notes" class="card-note-preview">{{ c.notes }}</div></div></div></div></div>
    </div>
    <div v-if="floatingReference" class="floating-modal-overlay" @click.self="floatingReference = null"><div class="floating-modal"><div class="float-header" :style="{ background: CONFIG.primaryColor }"><h4>{{ floatingReference.displayTitle }}</h4><button @click="floatingReference = null">✕</button></div><div class="float-body"><p class="float-content-text">{{ floatingReference.content }}</p><div v-if="floatingReference.notes" class="float-note-preview">{{ floatingReference.notes }}</div><button v-if="floatingReference.canJump" @click="selectClause(floatingReference); floatingReference = null" class="btn-jump-main" :style="{ background: CONFIG.primaryColor }">詳細內容與編輯 ➔</button><button v-else @click="floatingReference = null" class="btn-jump-main" style="background: #94a3b8">關閉</button></div></div></div>
  </div>
</template>
...
.mapping-tool h4 { margin: 0 0 5px 0; font-size: 13px; }
.mapping-desc { font-size: 11px; color: #64748b; margin-bottom: 10px; }
.map-list { display: flex; flex-direction: column; gap: 5px; margin-bottom: 10px; max-height: 160px; overflow-y: auto; }
.map-item { font-size: 12px; background: white; padding: 4px 8px; border-radius: 4px; display: flex; justify-content: space-between; }
.map-add-form { display: grid; grid-template-columns: 1fr 1fr auto; gap: 5px; }
.map-add-form input, .map-add-form select { font-size: 11px; padding: 5px; border: 1px solid #cbd5e1; border-radius: 4px; width: 100%; }
.btn-add-map { font-size: 11px; background: #334155; color: white; border: none; border-radius: 4px; padding: 0 10px; cursor: pointer; }
...
.content-box { background: white; padding: 35px; border-radius: 16px; border: 1px solid #e2e8f0; line-height: 1.8; font-size: 18px; margin-bottom: 30px; }
.content-text { white-space: pre-wrap; color: #334155;}
:deep(.ref-btn) { background: var(--bg-active); color: var(--primary); border: none; padding: 2px 8px; border-radius: 4px; font-size: 15px; font-weight: 800; cursor: pointer; margin: 0 4px; vertical-align: baseline;}
...
.card-num { font-weight: 800; color: var(--primary); text-align: center; }
.card-main { flex: 1; padding: 25px; }
.card-note-preview { margin-top: 15px; padding: 12px 15px; background: #fffbeb; border-radius: 8px; font-size: 14px; border-left: 4px solid #fbbf24; white-space: pre-wrap; }
...
.floating-modal { width: 450px; background: white; border-radius: 20px; overflow: hidden; animation: popUp 0.3s; }
.float-header { padding: 18px 25px; color: white; display: flex; justify-content: space-between; }
.float-body { padding: 25px; max-height: 500px; overflow-y: auto; }
.float-content-text { line-height: 1.8; color: #334155; margin-bottom: 20px; white-space: pre-wrap;}
.float-note-preview { background: #fef9c3; padding: 15px; border-radius: 12px; font-size: 14px; border-left: 4px solid #fbbf24; white-space: pre-wrap; margin-bottom: 10px;}
...
@media (max-width: 768px) {
  ...
  .card-side { width: 100%; flex-direction: row; justify-content: space-between; }
...
const LAW_TABLE_MAP = {
  ...
  '校園霸凌防制準則': 'campus_bullying_prevention_guidelines_clauses',
  '解聘不續聘停聘或資遣辦法': 'teachers_dismissal_regulations_clauses',
  '高級中等以下學校教師解聘不續聘停聘或資遣辦法': 'teachers_dismissal_regulations_clauses'
};
...
const uniqueLawNames = computed(() => {
  const names = new Set(Object.keys(LAW_TABLE_MAP));
  names.delete(CONFIG.pageTitle); // 排除自己，因為已經有【自身】選項
...
const parseContentWithLinks = (text) => { 
  ...
  let currentContextKeyword = '本辦法'; // 預設語境字眼
  const parts = text.split(articleRegex);
  
  return parts.map(part => {
    if (/^第\s*[0-9一二三四五六七八九十百千-]+\s*條/.test(part)) {
      // 🌟 將解析到的原始字面(Keyword)放入 data-law
      ...
      if (lawMatches.length > 0) {
        ...
        if (!['方法', '無法', '依法', '合法', '修法', '用法'].includes(lastLaw)) {
          currentContextKeyword = lastLaw; // 記錄精確的詞彙 (如 "本法", "該法")
        }
      }
      return part; 
    }
...
const handleContentClick = (e) => { 
  if (e.target.classList.contains('ref-btn')) { 
    ...
    const keyword = e.target.getAttribute('data-law'); // 抓出原本的字，如「本法」
    
    let targetLawName = '';

    // 🌟 1. 字典優先：如果字典有設定這個詞的對應，強制覆寫！
    if (customLawMap.value[keyword]) {
      targetLawName = customLawMap.value[keyword];
    } else {
      // 🌟 2. 預設語境判斷
      if (['本法', '該法', '同法', '前法'].includes(keyword)) targetLawName = CONFIG.parentLawName;
      else if (['本辦法', '本細則', '本準則'].includes(keyword)) targetLawName = CONFIG.pageTitle;
      else targetLawName = keyword;
    }
    ...
    if (targetLawName === CONFIG.pageTitle) {
      ...
      if (targetTable && parentClausesMap.value[targetLawName]) {
        ...
        alert(`📚 系統找不到「${targetLawName}」的資料庫。\n如果這是系統誤判，請在左側的【校正字典】中，將「${keyword}」對應到正確的法律！`);
        return;
      }
    }
          ...
          <div class="tools-panel">
            ...
            <div class="mapping-tool">
              ...
              <div class="map-add-form">
                <input v-model="newMapWrong" placeholder="錯誤字(如:本法)" class="map-input" />
