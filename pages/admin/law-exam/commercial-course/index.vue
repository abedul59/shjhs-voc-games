...
const currentSubject = '商事法'; // 🌟 強制對應資料庫欄位
  ...
  <div class="course-grid-container">
    <div class="header">
      ...
      <div class="title-area">
        <span class="subject-tag comm-tag">{{ currentSubject }}</span>
    ...
    <div v-else class="grid-layout">
      <div v-for="item in gridItems" :key="item.lessonNumber" class="grid-card" :class="{ 'has-data': item.hasData }">
        ...
        <div class="card-action">
          <NuxtLink v-if="item.hasData" :to="`/admin/law-exam/commercial-course/${item.id}`" class="action-btn view-btn">📖 閱讀</NuxtLink>
          <NuxtLink v-else :to="`/admin/law-exam/commercial-course/edit?lesson=${item.lessonNumber}`" class="action-btn add-btn">➕ 新增</NuxtLink>
        </div>
      </div>
    </div>
  </div>
...
.import-btn { background: #f0fdfa; color: #0f766e; border: 1px solid #5eead4; }
.subject-tag.comm-tag { background: #0f766e; color: white; padding: 6px 14px; border-radius: 8px; font-weight: 900; }
...
.grid-card.has-data { border-color: #5eead4; background: #f0fdfa; }
.grid-card.has-data:hover { border-color: #0f766e; transform: translateY(-5px); }
...
.view-btn { background: #0f766e; color: white; }
.add-btn { background: white; color: #0f766e; border: 1px solid #0f766e; }
</style>
