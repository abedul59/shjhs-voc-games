...
const CONFIG = {
  tableName: 'children_and_youth_welfare_act_clauses',
  pageTitle: '兒童及少年福利與權益保障法',
  parentLawName: '兒童及少年福利與權益保障法', // 母法為自身
  primaryColor: '#d97706', hoverColor: '#b45309', lightBg: '#fffbeb', activeBg: '#fde68a'
};

const supabase = useSupabaseClient();
const clauses = ref([]); const parentClausesMap = ref({}); const customLawMap = ref({}); 
const isLoading = ref(true); const viewMode = ref('single'); 
const selectedChapter = ref(null); const selectedSection = ref(null); 
...
const fetchData = async () => {
  ...
  if (data) {
    clauses.value = data.map(c => ({ ...c, urls: typeof c.urls === 'string' ? JSON.parse(c.urls||'[]') : c.urls })).sort((a, b) => parseFloat(String(a.article_num||'0').replace('-','.')) - parseFloat(String(b.article_num||'0').replace('-','.')));
...
const groupedClauses = computed(() => {
  const groups = {}; const data = searchQuery.value ? clauses.value.filter(c => (c.title||'').includes(searchQuery.value) || (c.content||'').includes(searchQuery.value)) : clauses.value;
  data.forEach(c => { const chapter = c.chapter_name || '未分類編'; const section = c.section_name || '未分類章'; if (!groups[chapter]) groups[chapter] = {}; if (!groups[chapter][section]) groups[chapter][section] = []; groups[chapter][section].push(c); });
  return groups;
});
...
const chapterClauses = computed(() => filteredFlatClauses.value.filter(c => { const matchChap = (c.chapter_name||'未分類編') === selectedChapter.value; return selectedSection.value ? matchChap && (c.section_name||'未分類章') === selectedSection.value : matchChap; }));
...
const getNormalizedArticleNum = (rawText) => { const match = rawText.replace(/\s/g, '').match(/第(.+)(?:條|點)(?:之(.+))?/); return match ? parseNum(match[1]) + (match[2] ? '-' + parseNum(match[2]) : '') : null; };
...
const handleContentClick = (e) => { 
  if (e.target.classList.contains('ref-btn')) { 
    ...
    if (['self', '本法', '本條例'].includes(inheritedContext)) finalTargetLaw = CONFIG.pageTitle;
    else if (['該法', '同法', '前法'].includes(inheritedContext)) finalTargetLaw = CONFIG.parentLawName;
    ...
    else { const db = parentClausesMap.value[finalTargetLaw]; if (db) { target = db.find(c => String(c.article_num) === String(convertedNum)); displayTitle = target ? `【${finalTargetLaw}】${target.title}` : ''; } else { alert(`📚 找不到「${finalTargetLaw}」的資料。請在左側校正字典對應！`); return; } }
...
<template>
  <div class="law-layout" :style="cssVars">
    <div class="mobile-nav"><button class="btn-menu" @click="showSidebar = true">☰ 目錄</button><div class="mode-toggle"><button @click="viewMode='single'" :class="{active: viewMode==='single'}">逐條</button><button @click="viewMode='chapter'" :class="{active: viewMode==='chapter'}">章節</button><button @click="viewMode='all'" :class="{active: viewMode==='all'}">全覽</button></div></div>
    <div class="sidebar" :class="{ 'mobile-open': showSidebar }">
      <div class="sidebar-header">
        ...
        <div class="header-top"><NuxtLink to="/admin/law-exam" class="back-link">← 回專區</NuxtLink><div class="mode-toggle desktop-only"><button @click="viewMode='single'" :class="{active: viewMode==='single'}">逐條</button><button @click="viewMode='chapter'" :class="{active: viewMode==='chapter'}">章節</button><button @click="viewMode='all'" :class="{active: viewMode==='all'}">全覽</button></div></div>
        <details class="admin-tools"><summary>⚙️ 管理與校正字典</summary><div class="tools-panel"><div class="top-actions"><label class="btn-tool primary">📥 匯入<input type="file" @change="handleImport" hidden/></label><button @click="exportCSV" class="btn-tool">📤 匯出</button></div></div></details>
      ...
      <div class="tree-list">
        ...
        <div v-else v-for="(sections, chapter) in groupedClauses" :key="chapter" class="chapter-group">
          <details open><summary class="chapter-title"><span>{{ chapter }}</span><button @click.prevent="viewChapter(chapter)" class="btn-view-group">讀此編</button></summary>
            <div v-for="(items, section) in sections" :key="section" class="section-group">
              <details open><summary class="section-title"><span>{{ section }}</span><button @click.prevent="viewSection(chapter, section)" class="btn-view-group">讀此節</button></summary>
                <div v-for="c in items" :key="c.id" class="clause-item" :class="{active: selectedClause?.id === c.id}"><span @click="selectClause(c)" class="clause-label">{{ c.title }}</span></div>
              </details>
            </div>
          </details>
        </div>
    ...
    <div class="main-content">
      <div v-if="viewMode === 'single'">
        ...
        <div v-else class="clause-detail">
          <div class="clause-header"><button @click="goBackToChapter" class="btn-back-inner">← 返回所在章節</button><div class="breadcrumb">{{selectedClause.chapter_name}} > {{selectedClause.section_name}}</div><h1>{{ selectedClause.title }}</h1></div>
      ...
      <div v-else-if="viewMode === 'chapter'">
        ...
        <div v-else class="full-text-view">
          ...
          <div class="clauses-container"><div v-for="c in chapterClauses" :key="c.id" class="full-clause-card"><div class="card-side"><span class="card-num">{{ c.title }}</span><button @click="selectClause(c)" class="btn-jump-edit">📝 編輯</button></div><div class="card-main"><div class="card-content" v-html="parseContentWithLinks(c.content)" @click="handleContentClick"></div><div v-if="c.notes" class="card-note-preview">{{ c.notes }}</div></div></div></div>
        </div>
      </div>
      <div v-else class="full-text-view">
...
.btn-tool { flex: 1; padding: 6px; border: 1px solid #cbd5e1; border-radius: 8px; font-size: 11px; font-weight: bold; text-align: center; cursor: pointer;}
...
.clause-item { display: flex; align-items: center; gap: 8px; padding: 6px 20px; border-radius: 6px; }
