...
const groupedClauses = computed(() => {
  ...
  data.forEach(c => { const chap = c.chapter_name || '未分類編'; const sec = c.section_name || '未分類章'; if (!groups[chap]) groups[chap] = {}; if (!groups[chap][sec]) groups[chap][sec] = []; groups[chap][sec].push(c); });
  return groups;
});
...
const chapterClauses = computed(() => filteredFlatClauses.value.filter(c => { const m = (c.chapter_name||'未分類編') === selectedChapter.value; return selectedSection.value ? m && (c.section_name||'未分類章') === selectedSection.value : m; }));
...
const viewChapter = (chap) => { selectedChapter.value = chap; selectedSection.value = null; viewMode.value = 'chapter'; showSidebar.value = false; };
const viewSection = (chap, sec) => { selectedChapter.value = chap; selectedSection.value = sec; viewMode.value = 'chapter'; showSidebar.value = false; };
...
const handleContentClick = (e) => { 
  if (e.target.classList.contains('ref-btn')) { 
    ...
    if (['self', '本法', '本條例'].includes(inherited)) final = CONFIG.pageTitle;
    else if (['該法', '同法', '前法'].includes(inherited)) final = CONFIG.parentLawName;
...
const batchDelete = async () => { if (!confirm(`確定刪除選中的 ${selectedIds.value.length} 項？`)) return; await supabase.from(CONFIG.tableName).delete().in('id', selectedIds.value); selectedIds.value = []; fetchData(); };
...
const saveManual = async (c) => { isSaving.value = true; await supabase.from(CONFIG.tableName).update({ notes: c.notes }).eq('id', c.id); isSaving.value = false; alert('儲存成功'); };
</script>
    ...
    <div class="sidebar" :class="{ 'mobile-open': showSidebar }">
      <div class="sidebar-header">
        ...
        <details class="admin-tools"><summary>⚙️ 管理與校正</summary><div class="tools-panel">
          ...
          <div class="mapping-tool"><h4>🔗 條文指涉校正</h4>
        ...
        <input v-model="searchQuery" class="search-input" placeholder="🔍 搜尋關鍵字..." />
      </div>
      <div class="tree-list">
        <div v-if="isLoading" class="list-msg">載入中...</div>
        <div v-else v-for="(secs, chap) in groupedClauses" :key="chap" class="chapter-group">
          <details open><summary class="chapter-title"><span>{{ chap }}</span><button @click.prevent="viewChapter(chap)" class="btn-view-group">讀此編</button></summary>
            <div v-for="(items, sec) in secs" :key="sec" class="section-group">
              <details open><summary class="section-title"><span>{{ sec }}</span><button @click.prevent="viewSection(chap, sec)" class="btn-view-group">讀此節</button></summary>
                <div v-for="c in items" :key="c.id" class="clause-item" :class="{active: selectedClause?.id === c.id}"><input type="checkbox" v-model="selectedIds" :value="c.id"/><span @click="selectClause(c)" class="clause-label">{{ c.title }}</span></div>
              </details>
            </div>
          </details>
        </div>
    ...
    <div class="main-content">
      <div v-if="viewMode === 'single'">
        ...
        <div v-else class="clause-detail">
          <div class="clause-header"><button @click="goBackToChapter" class="btn-back-inner">← 返回所在章節</button><h1>{{ selectedClause.title }}</h1></div>
      ...
      <div v-else class="full-text-view">
        <div class="view-header"><h1>{{ viewMode === 'chapter' ? selectedChapter : '全部條文' }}</h1></div>
        <div class="clauses-container"><div v-for="c in (viewMode === 'chapter' ? chapterClauses : filteredFlatClauses)" :key="c.id" class="full-clause-card"><div class="card-side"><span>{{ c.title }}</span><button @click="selectClause(c)" class="btn-jump-edit">編輯</button></div><div class="card-main"><div class="card-content" v-html="parseContentWithLinks(c.content)" @click="handleContentClick"></div></div></div></div>
      </div>
    </div>
