...
const currentSubject = '智慧財產權法'; // 🌟 強制對應資料庫欄位
  ...
  <div class="course-grid-container">
    <div class="header">
      ...
      <div class="title-area">
        <span class="subject-tag ip-tag">{{ currentSubject }}</span>
    ...
    <div v-else class="grid-layout">
      <div v-for="item in gridItems" :key="item.lessonNumber" class="grid-card" :class="{ 'has-data': item.hasData }">
        ...
        <div class="card-action">
          <NuxtLink v-if="item.hasData" :to="`/admin/law-exam/ip-course/${item.id}`" class="action-btn view-btn">📖 閱讀</NuxtLink>
          <NuxtLink v-else :to="`/admin/law-exam/ip-course/edit?lesson=${item.lessonNumber}`" class="action-btn add-btn">➕ 新增</NuxtLink>
        </div>
      </div>
    </div>
  </div>
...
.import-btn { background: #f5f3ff; color: #7c3aed; border: 1px solid #c4b5fd; }
.subject-tag.ip-tag { background: #7c3aed; color: white; padding: 6px 14px; border-radius: 8px; font-weight: 900; }
...
.grid-card.has-data { border-color: #c4b5fd; background: #f5f3ff; }
.grid-card.has-data:hover { border-color: #7c3aed; transform: translateY(-5px); }
...
.view-btn { background: #7c3aed; color: white; }
.add-btn { background: white; color: #7c3aed; border: 1px solid #7c3aed; }
</style>
