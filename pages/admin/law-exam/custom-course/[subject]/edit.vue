...
const subjectName = route.params.subject; // 🌟 自動從網址抓取科目名稱
const courseMeta = ref({ icon: '📚', theme_color: '#3b82f6' }); // 預設藍色防呆
...
const form = ref({
  id: null, subject: subjectName, topic: '', introduction: '', class_video_link: '', notebook_audio_link: '', document_link: '', notebook_infographic_link: '', notebook_quiz_link: '', notebook_flashcard_link: '', notebook_overall_link: '', summary: '', transcript: '', personal_notes: '', associated_urls: ''
});
...
const handleSave = async () => {
  ...
  if (!form.value.topic.trim()) { alert('請填寫講次/主題！'); return; }
  ...
  if (error) { alert('儲存失敗：' + error.message); } else { form.value.id = data.id; alert('✅ 儲存成功！'); syncEditorsToForm(); }
...
<template>
  <!-- 🌟 將資料庫抓到的顏色設為 CSS 變數，後綴 '1A' 代表 10% 透明度作為淺色背景 -->
  <div class="edit-container" :style="{ '--theme': courseMeta.theme_color, '--theme-light': courseMeta.theme_color + '1A' }">
    <div class="header">
      <button @click="router.back()" class="back-btn">← 取消並返回</button>
      <div class="title-wrap">
        <h1>筆記編輯器</h1>
        <span class="subject-tag" style="background: var(--theme);">{{ courseMeta.icon }} {{ subjectName }}</span>
      </div>
    </div>
    ...
    <div v-else class="form-body">
      <section v-show="activeTab === 'basic'" class="tab-content">
        <div class="form-section">
          <h3>講次基本資訊</h3>
          <div class="form-group"><label>主題名稱 (必填)</label><input v-model="form.topic" type="text" class="input-field" placeholder="請輸入主題名稱" /></div>
          <div class="form-group"><label>簡介</label><textarea v-model="form.introduction" rows="3" class="input-field"></textarea></div>
        </div>
        <div class="form-section link-grid-section">
          <h3>🤖 資源連結</h3>
          <div class="link-grid">
            <div class="form-group"><label>▶️ 影片</label><input v-model="form.class_video_link" class="input-field" /></div>
            <div class="form-group"><label>🎧 聲音</label><input v-model="form.notebook_audio_link" class="input-field" /></div>
        ...
        <div class="form-section">
          ...
          <div v-for="(note, index) in personalNotesList" :key="index" class="personal-note-wrapper">
            <div class="note-header">
              <span class="note-badge">區塊 {{ index + 1 }}</span>
              <button @click.prevent="removePersonalNote(index)" class="btn-remove-note">🗑️</button>
            </div>
            <div :id="'personal-note-' + index" contenteditable="true" class="rich-textarea input-field personal-bg" placeholder="在此整理筆記..."></div>
          </div>
          <button @click.prevent="addPersonalNote" class="btn-add-note">➕ 新增筆記區塊</button>
          <hr class="divider">
          <div class="form-group">
            <label>🔗 關聯網址 (自動編號)</label>
            <div ref="urlsRef" contenteditable="true" class="rich-textarea input-field" placeholder="貼上網址..."></div>
          </div>
        </div>
        <div class="tab-actions"><button @click="handleSave" class="save-btn" :disabled="isSaving">💾 儲存</button></div>
      </section>
...
.subject-tag { color: white; padding: 4px 10px; border-radius: 6px; font-size: 13px; font-weight: bold; }
...
.tabs-nav button.active { background: var(--theme); color: white; }
...
.form-section h3 { margin: 0 0 20px 0; font-size: 18px; color: #1e293b; border-left: 5px solid var(--theme); padding-left: 12px; }
...
.note-badge { background: #1e293b; color: white; padding: 4px 10px; border-radius: 6px; font-size: 12px; font-weight: bold; }
...
/* 🌟 動態套用背景顏色 */
.personal-bg { background: var(--theme-light); border-color: var(--theme); min-height: 200px;}
.btn-add-note { display: block; width: 100%; padding: 12px; background: var(--theme-light); color: var(--theme); border: 2px dashed var(--theme); border-radius: 10px; font-weight: bold; font-size: 15px; cursor: pointer; transition: 0.2s;}
.btn-add-note:hover { opacity: 0.8; }
...
.save-btn { background: var(--theme); color: white; border: none; padding: 15px 40px; border-radius: 12px; font-size: 16px; font-weight: bold; cursor: pointer; box-shadow: 0 4px 10px rgba(0,0,0,0.1); }
