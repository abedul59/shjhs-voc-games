...
const addCustomMap = async () => {
  ...
  const wrong = newMapWrong.value.trim().replace(/\s/g, ''); // 存入字典時自動清空白
...
const handleContentClick = (e) => { 
  if (e.target.classList.contains('ref-btn')) { 
    ...
    // 🌟 2. 雙重攔截校正
    if (customLawMap.value[cleanRawText]) {
      // 優先攔截：檢查「條文字面」(如: 第十六條) 
      targetLawName = customLawMap.value[cleanRawText];
    } else if (customLawMap.value[targetLawName]) {
      // 次要攔截：檢查「語境代稱」(如: 本法, 教育基本法)
    ...
    if (targetLawName === CONFIG.pageTitle) {
      ...
      if (targetTable && parentClausesMap.value[targetLawName]) {
        ...
        alert(`📚 系統找不到「${targetLawName}」的資料庫。\n如果這是系統誤判，請在左側的【校正字典】中，將「${cleanRawText}」或「${e.target.getAttribute('data-law')}」對應到正確的法律！`);
        return;
      }
    }
