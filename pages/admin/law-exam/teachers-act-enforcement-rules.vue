...
const clauses = ref([]);
const parentClauses = ref([]); // 🌟 專門用來存放「母法(教師法)」的資料
...
const fetchData = async () => {
  ...
  const [childRes, parentRes] = await Promise.all([
    ...
    supabase.from('teachers_act_clauses').select('*') // 抓取母法
  ]);

  if (childRes.error) alert('無法連線細則資料庫：' + childRes.error.message);
  if (parentRes.error) console.error('無法連線母法資料庫');
  ...
  if (parentRes.data) {
    parentClauses.value = parentRes.data; // 儲存母法資料
  }
...
const parseContentWithLinks = (text) => { 
  if (!text) return ''; 
  // 正則表達式現在會抓取前面有「本法」或「教師法」的條文
  const regex = /((?:本法|教師法)?第\s*[0-9一二三四五六七八九十百千-]+[條之]*[0-9一二三四五六七八九十百千-]*\s*條)/g; 
  return text.split(regex).map(part => {
    if (regex.test(part)) {
      // 判斷是不是母法
      const isParentLaw = part.includes('本法') || part.includes('教師法');
      const targetType = isParentLaw ? 'parent' : 'self';
      return `<button class="ref-btn" data-type="${targetType}" data-raw="${part}">${part}</button>`;
    }
    return part;
  }).join(''); 
};
...
const handleContentClick = (e) => { 
  if (e.target.classList.contains('ref-btn')) { 
    ...
    if (targetType === 'parent') {
      // 如果點擊的是「本法」，去母法資料庫找
      target = parentClauses.value.find(c => c.article_num === convertedNum);
      if(target) displayTitle = `【教師法】${target.title}`;
    } else {
      // 如果是一般的「第X條」，在施行細則自己裡面找
      target = clauses.value.find(c => c.article_num === convertedNum);
      if(target) displayTitle = target.title;
    }

    if (target) {
      // 賦予顯示用標題
      floatingReference.value = { ...target, displayTitle: displayTitle, targetType: targetType };
    } else {
      alert(`無法定位：${rawText} (條號 ${convertedNum})。請確認母法資料已匯入。`);
    }
  } 
};
  ...
  <div class="law-layout">
    ...
    <div v-if="floatingReference" class="floating-modal-overlay" @click.self="floatingReference = null">
      <div class="floating-modal">
        <div class="float-header" :style="{ background: CONFIG.primaryColor }">
          <h4>{{ floatingReference.displayTitle }} 參照</h4>
        ...
        <div class="float-body">
          ...
          <button v-if="floatingReference.targetType === 'self'" @click="selectClause(floatingReference); floatingReference = null" class="btn-jump-main" :style="{ background: CONFIG.primaryColor }">詳細內容與編輯 ➔</button>
