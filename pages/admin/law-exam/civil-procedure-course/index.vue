...
const currentSubject = '民事訴訟法'; // 🌟 強制對應資料庫欄位
  ...
  <div class="course-grid-container">
    <div class="header">
      ...
      <div class="title-area">
        <span class="subject-tag cp-tag">{{ currentSubject }}</span>
    ...
    <div v-else class="grid-layout">
      <div v-for="item in gridItems" :key="item.lessonNumber" class="grid-card" :class="{ 'has-data': item.hasData }">
        ...
        <div class="card-action">
          <NuxtLink v-if="item.hasData" :to="`/admin/law-exam/civil-procedure-course/${item.id}`" class="action-btn view-btn">📖 閱讀</NuxtLink>
          <NuxtLink v-else :to="`/admin/law-exam/civil-procedure-course/edit?lesson=${item.lessonNumber}`" class="action-btn add-btn">➕ 新增</NuxtLink>
        </div>
      </div>
    </div>
  </div>
...
.import-btn { background: #fffbeb; color: #d97706; border: 1px solid #fcd34d; }
.subject-tag.cp-tag { background: #d97706; color: white; padding: 6px 14px; border-radius: 8px; font-weight: 900; }
...
.grid-card.has-data { border-color: #fcd34d; background: #fffbeb; }
.grid-card.has-data:hover { border-color: #d97706; transform: translateY(-5px); }
...
.view-btn { background: #d97706; color: white; }
