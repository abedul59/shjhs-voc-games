...
onMounted(async () => {
  ...
  // 1. 取得學生測驗紀錄 (從 exam2_history 抓取題組測驗總紀錄)
  const { data: hData } = await supabase.from('exam2_history').select('*').order('created_at', { ascending: false });
  if (hData) historyData.value = hData;

  // 2. 取得所有題組題目 (從 exam2_questions 抓取)
  const { data: qData } = await supabase.from('exam2_questions').select('id, year, group_id, question_num, question_text');
  
  // 3. 取得所有子題的作答日誌 (從 exam2_question_logs 抓取)
  const { data: lData } = await supabase.from('exam2_question_logs').select('q_id, is_correct, time_spent');
...
<template>
  <div class="admin-container">
    ...
    <div v-else-if="activeTab === 'history'" class="card">
      <p style="color:#555; margin-bottom: 15px;">※ 這裡紀錄學生挑戰特定「年份題組」的完整成績。</p>
    ...
    <div v-else-if="activeTab === 'items'" class="card">
      <p style="color:#555; margin-bottom: 15px;">※ 這裡統整了「題組」內每個子題的作答狀況。列表已依據<strong>「答對率 (由低到高)」</strong>排序，幫您快速抓出學生最常錯的子題！</p>
      <div class="table-wrapper">
        <table>
          <thead>
            <tr>
              <th width="120">年份 - 題組</th>
              <th width="60">題號</th>
              <th>題目預覽</th>
              <th width="80">作答總數</th>
          ...
          <tbody>
            <tr v-for="q in itemStats" :key="q.id">
              <td style="font-weight: bold; color: #1565c0;">{{ q.year }} - {{ q.group_id }}</td>
              <td style="font-weight: bold; font-size: 1.1rem;">{{ q.question_num }}</td>
              <td class="text-left" style="font-size: 0.9rem; color: #444;">{{ q.question_text.substring(0, 50) }}...</td>
              <td>{{ q.total }}</td>
            ...
            <tr v-if="itemStats.length === 0"><td colspan="8">尚無任何子題作答數據</td></tr>
          </tbody>
        </table>
      </div>
    </div>
