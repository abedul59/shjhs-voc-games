...
const getPlayerName = (id) => {
  ...
  if (!s) return '未知玩家';
  return `${s.class_name} - ${s.real_name || s.hidden_name}`;
};
</script>

<template>
  <div class="admin-lb-container">
    <div class="header-box retro-element"><h1>🏆 班級英雄榜 (管理員)</h1><p>CLASS LEADERBOARD</p></div>
    <div class="top-nav">
    ...
    <div class="filter-box retro-element">
      ...
      <div class="form-group" style="margin-top: 15px;">
        <select v-model="selectedClass" @change="fetchLeaderboard" class="retro-input class-select" v-if="identityMode === 'student'">
    ...
    <div class="rank-list" v-if="rankedList.length > 0">
      <div class="rank-card retro-element" v-for="(record, index) in rankedList" :key="record.id" :class="{'top-1': index===0, 'top-2': index===1, 'top-3': index===2}">
        ...
        <div class="rank-score" v-else>
          <strong>{{ record.score }}</strong> 分<br><small>⏱️ {{ record.computedTime }}s</small>
        </div>
...
<style scoped>
.admin-lb-container { padding: 20px; box-sizing: border-box; max-width: 800px; margin: 0 auto; }
...
.type-btn, .id-btn { flex: 1; min-width: 12%; padding: 8px 5px; font-size: 0.85rem; font-weight: 900; background: var(--box-bg); color: var(--text-main); border: var(--border-width) solid var(--border-color); border-radius: var(--radius-element); cursor: pointer; box-shadow: var(--shadow-btn); transition: all 0.2s; }
